...
%                             TTickScale units
% RTickOffset       {.02} Normalized radial offset for rho tick labels
...
% TTickOffset       {.06} normalized radial offset of theta tick labels
...
%suppresses silly divide-by-zero
...
if HoldIsON % a current held plot exists
   ...
   D=getappdata(HAxes,'FFPP_Data');      % get stored data
   P=getappdata(HAxes,'FFPP_Properties');
   ...
   D.LineWidth=[D.LineWidth; get(Hlines,{'LineWidth'})];
   D.NumLines=length(D.TData);
   
   delete(Hlines) % got the data, lines are no longer needed
   
   set(tmpaxes,'NextPlot','add') % hold on
   for k=1:D.NumLines % plot all data with theta in degrees
      plot(tmpaxes,D.TData{k}*180/pi,D.RData{k},...
         'Color',D.LineColor{k},...
   ...
   P.TLimit=get(tmpaxes,'XLim');             % Theta axis limits in degrees
   P.TTickValue=get(tmpaxes,'XTick');            % Default Theta axis ticks
   ...
   delete(D.HPlot)           % clear cartesian axes, then create polar axes
   ...
   try % the plot function should work now
      ...
      D.HPlot=plot(HAxes,varargin{:});
   catch
      error('Input Arguments Not Understood.')
   end
   HFig=ancestor(HAxes,'figure');
   D.NumLines=length(D.HPlot);     % get all data for storage
   D.TData=get(D.HPlot,{'XData'});
   D.RData=get(D.HPlot,{'YData'});
   D.LineColor=get(D.HPlot,{'Color'});
   D.LineStyle=get(D.HPlot,{'LineStyle'});
   D.Marker=get(D.HPlot,{'Marker'});
   D.LineWidth=get(D.HPlot,{'LineWidth'});
   ...
   for k=1:D.NumLines                              % Condition plotted data
      % scale theta data to degrees
      set(D.HPlot(k),'XData',(180/pi)*D.TData{k})
      % normalize rho data for plotting
      D.RDataN(k)={(D.RData{k}-D.RMin)/D.RLimitDiff};
   end
   P.TTickValue=get(HAxes,'XTick');              % Default Theta axis ticks
   P.TLimit=get(HAxes,'XLim');                % get theta limits in degrees
   [P,D]=local_getTTickValue(P,D);            % get theta tick values
   delete(D.HPlot)           % clear cartesian axes, then create polar axes
...
for k=1:D.NumLines % plot the normalized data
   ...
   D.HPlot(k)=plot(HAxes,xdata,ydata,...
                  'Color',D.LineColor{k},...
                  'LineStyle',D.LineStyle{k},...
                  'Marker',D.Marker{k},...
                  'LineWidth',D.LineWidth{k});
...
setappdata(HAxes,'FFPP_Properties',P)
setappdata(HAxes,'FFPP_Data',D)
...
for k=1:length(Sfn)
   ...
   case 'RLimit'                                                   % RLimit
      if isnumeric(S.RLimit) && numel(S.RLimit)==2
         ...
         for k=1:length(D.RData)
            ...
            set(D.HPlot(k),'XData',xdata,'YData',ydata)
         end
      else
   ...
   case 'RTickAngle'                                           % RTickAngle
      if isnumeric(S.RTickAngle) && isscalar(S.RTickAngle)
         ...
         if rad>=P.TLimit(1) && rad<=P.TLimit(2)
   ...
   case 'TLimit'                                                   % TLimit
      if isnumeric(S.TLimit) && numel(S.TLimit)==2
         S.TLimit=[min(S.TLimit) max(S.TLimit)];
         ...
         [P,D]=local_placeRTickLabel(HAxes,P,D,S);
         for k=1:length(D.TData) % hide data outside limits
            tdata=(mod(D.TData{k}*180/pi+180,360)-180)*pi/180;
   ...
   case 'TTickValue'                                           % TTickValue
      if isnumeric(S.TTickValue) && numel(S.TTickValue)>0
         ...
         TT= TTick>=P.TLimit(1) & TTick<=P.TLimit(2);
         S.TTickValue=S.TTickValue(TT);
         if length(S.TTickValue)>1
            P.TTickValue=S.TTickValue(:)';
            D.TTickValue=TTick(TT);
...
idx=strmatch(lower(str),lower(fnames),'exact'); % check for exact match
...
% get TTicks
if nargin==2 % new plot
   TLimit=P.TLimit/90;                        % find quadrant limits are in
   TLimit=[floor(TLimit(1)) ceil(TLimit(2))]; % choose nearest quadrant
   D.NumQuad=abs(diff(TLimit));
   if D.NumQuad>2
      TLimit=[-2 2];
      D.NumQuad=4;
   end
   P.TLimit=TLimit*pi/2; % convert theta axis limits to radians
elseif isfield(S,'TLimit') % update current plot
   TLimit=abs(diff(S.TLimit))*2/pi;
   D.NumQuad=max(1,round(TLimit));
   if D.NumQuad>=4
      S.TLimit=[-1 1]*pi;
      D.NumQuad=4;
   end
...
if strcmp(P.TTickScale,'degrees') % ticks are in degrees
   TDelta=[10 15 15 30];
   P.TTickValue=-360:TDelta(D.NumQuad):360; % possible ticks
   k=(P.TTickValue>=floor(P.TLimit(1)*180/pi))...
                  & (P.TTickValue<=ceil(P.TLimit(2)*180/pi));
   P.TTickValue=P.TTickValue(k);
   P.TTickValue=unique(rem(P.TTickValue,360)); % get unique ticks
   if any(P.TTickValue==-180) && any(P.TTickValue==180)
      P.TTickValue(P.TTickValue==180)=[];
   end
   D.TTickValue=P.TTickValue*pi/180; % store ticks in radians
else                          % ticks are in radians
   TDelta=[5 10 10 15]; % *pi/100
   P.TTickValue=(-200:TDelta(D.NumQuad):200)*pi/100; % possible ticks
   P.TTickValue=P.TTickValue(P.TTickValue>=P.TLimit(1)...
                           & P.TTickValue<=P.TLimit(2));
   P.TTickValue=unique(rem(P.TTickValue,2*pi)); % get unique ticks
   tmp=abs(abs(P.TTickValue)-pi)<pi/100;
   if sum(tmp)==2
      P.TTickValue((P.TTickValue-pi)<pi/100)=[];
   end
   D.TTickValue=P.TTickValue; % store ticks in radians for plotting
end
D.TTickLabelN=length(D.TTickValue);
if nargin==2 || D.RTickAngle<P.TLimit(1) || D.RTickAngle>P.TLimit(2)
...
theta=linspace(P.TLimit(1),P.TLimit(2),D.NumQuad*100);
costheta=cos(theta);
sintheta=sin(theta);
...
out.TTickOffset=0.06;
out.TTickDirection='in';
out.TTickLabel='';
out.TTickLabelVisible='on';
out.TTickValue=-180:30:150;
out.TTickVisible='on';
...
% D.NumQuad       Double      Number of quadrants in plot
